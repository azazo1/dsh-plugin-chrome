...
interface Harness {
  /** Run one chrome_* call through the gate with the given arguments; downstream decides `allow`. */
  call: (toolName: string, args?: unknown, downstream?: PreToolDecision) => Promise<PreToolDecision>
...
function harness(options: HarnessOptions = {}): Harness {
  ...
  return {
    consent,
    call: async (toolName, args = {}, downstream = { kind: 'allow' }) => {
      ...
      const exec = { name: toolName, arguments: args, ...agent !== undefined ? { agent } : {} } as unknown as ToolExecution
      return listener(exec, async () => downstream)
    },
  }
}

/** One chrome_open call's arguments, with the mandatory model-authored reason. */
const OPEN_ARGS = { justification: '打开内部管理后台核对订单状态' }

describe('chrome 首次启动的审批', () => {
  it('首次 chrome_open 询问用户', async () => {
    const gate = harness()
    const decision = await gate.call('chrome_open', OPEN_ARGS)
    expect(decision.kind).toBe('ask')
    expect(decision.kind === 'ask' ? decision.reason : '').toContain('首次')
  })

  it('审批理由就是模型给的 justification', async () => {
    const gate = harness()
    const decision = await gate.call('chrome_open', OPEN_ARGS)
    const reason = decision.kind === 'ask' ? decision.reason ?? '' : ''
    expect(reason).toContain(OPEN_ARGS.justification)
  })
  ...
  it('同意之后本会话不再询问', async () => {
    const gate = harness()
    expect((await gate.call('chrome_open', OPEN_ARGS)).kind).toBe('ask')
    gate.consent.grant('sess-0001') // 用户点了"允许一次", 工具体记录了同意
    expect((await gate.call('chrome_open', OPEN_ARGS)).kind).toBe('allow')
  ...
  it('隐式开窗的工具不带理由时被拒绝, 并提示改用 chrome_open', async () => {
    const gate = harness()
    const navigate = await gate.call('chrome_navigate')
    expect(navigate.kind).toBe('deny')
    expect(navigate.kind === 'deny' ? navigate.reason : '').toContain('chrome_open')
    expect((await gate.call('chrome_screenshot')).kind).toBe('deny')
    expect((await gate.call('chrome_status')).kind).toBe('allow')
    expect((await gate.call('chrome_close')).kind).toBe('allow')
  })

  it('隐式开窗的工具自己带上理由时照常询问', async () => {
    const gate = harness()
    const decision = await gate.call('chrome_navigate', { justification: '打开内部管理后台' })
    expect(decision.kind).toBe('ask')
    expect(decision.kind === 'ask' ? decision.reason : '').toContain('内部管理后台')
  })
  ...
  it('保留下游监听器的拒绝决定', async () => {
    const gate = harness()
    const decision = await gate.call('chrome_open', OPEN_ARGS, { kind: 'deny', reason: '被其他策略拒绝' })
